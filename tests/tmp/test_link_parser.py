#!/usr/bin/env python3
"""
Тестовый скрипт парсера ссылок - проверяет сбор ссылок на фильмы с LordFilm

📝 Назначение:
- Тестирует работу LinkParser на реальных страницах
- Проверяет сбор ссылок на фильмы и сохранение в БД
- Генерирует тестовые URL через URLGenerator
- Создает тестовую сессию парсинга для отслеживания прогресса

🎯 Что тестирует:
- Генерация URL страниц с фильмами
- Парсинг ссылок на отдельные фильмы
- Сохранение ссылок в базу данных
- Создание записей о сессии парсинга

⚠️  Особенности:
- Использует только ПЕРВЫЕ 2 страницы для теста
- Создает тестовую сессию в БД (не влияет на продакшен)
- Не парсит детали фильмов, только собирает ссылки
"""
import sys
import os
