        ...
        for page in range(1, pages + 1):
            ...
            try:
                ...
                for item in soup.select('.th-item'):
                    ...
                    # Parse country (from the info line)
                    info_elem = link_elem.select_one('.th-info')
                    country = None
                    if info_elem:
                        info_text = info_elem.get_text(strip=True)
                        # Try to extract country from info text (usually format: "country, genre1, genre2")
                        country_match = re.search(r'^([^,]+)', info_text)
                        if country_match:
...
if __name__ == "__main__":
    ...
    for sort_criteria, filename in [('kp', 'top_kp.json'),
        ...
        for i, movie in enumerate(top_movies, 1):
            ...
            print(
                f"{i}. {movie['title']} ({movie['year']}, {movie.get('country', 'N/A')}) - {rating_display} {movie['link']}")